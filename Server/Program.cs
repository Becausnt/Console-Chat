...
class ChatServer
{
    ...
    //「name」: message <- design for messages
    ...
    static void HandleServer()
    {
        try
        {
            while (true)
            {
                ...
                ThreadPool.QueueUserWorkItem(HandleClient, client); // start thread for every client
            }
        }
        catch (Exception e)
        {
    ...
    static void HandleServerConsoleCommands()
    {
        string command = Console.ReadLine();
        string commandUpper = command.ToUpper();
        if (!string.IsNullOrEmpty(command))
        {
            if (!string.IsNullOrEmpty(command.Substring(command.Split(" ")[0].Length))) // check if the command has an argument
            {
                switch (commandUpper.Split(' ')[0])
                {
                    case "/KICK":
                        ...
                        else
                        {
                            try
                            {
                                Broadcast("[SERVER]: Kicking all", String.Empty); // string.Empty because the Broadcast function needs a second argument
    ...
    static void HandleClient(object obj)
    {
        ...
        bool isNameTaken = false; // no duplicate names allowed
        bool isBanned = false;
        ...
        try
        {
            ...
            while ((bytesRead = stream.Read(buffer, 0, buffer.Length)) != 0)
            {
                ...
                if (message.StartsWith("/"))
                {
                        ...
                        switch (message.ToUpper().Split(' ')[0])
                        {
                            case "/KICK":
                                try
                                {
                                    ...
                                    clients.TryRemove(command.Substring(6), out _);
                                    Broadcast($"[SERVER]: {command.Substring(6)} was kicked.", String.Empty);
                                    Console.WriteLine($"[/KICK]: {clientName} kicked {command.Substring(6)}");
                                }
                                catch
                                {
                            ...
                            case "/SHADOWKICK":
                                try
                                {
                                    ...
                                    clients[command.Substring(12)].Close();
                                    clients.TryRemove(command.Substring(12), out _);
                                    Console.WriteLine($"[/SHADOWKICK]: {clientName} kicked {command.Substring(6)}");
                                }
                                catch
                                {
                                    SendMessage(client, $"[/SHADOWKICK]: Failed to kick {command.Substring(6)}");
                                }
                                break;
                            case "/BAN":
                                try
                                {
                                    ...
                                    Console.WriteLine($"[/BAN]: {clientName} banned {command.Substring(6)}");
                                }
                                catch
                                {
                            ...
                            case "/UNBAN":
                                if (!string.IsNullOrEmpty(command.Substring(6)))
                                {
                                    ...
                                    {
                                        bannedNames.Remove(command.Substring(7));
                                        Console.WriteLine($"[/UNBAN]: {clientName} unbanned {command.Substring(7)}");
                                    }
                            ...
                            case "/IPBAN":
                                try
                                {
                                    ...
                                    targetClient.Close();
                                    Console.WriteLine($"[/IPBAN]: {clientName} ipbanned {command.Substring(7)}");
                                }
                                catch
                                {
                            ...
                            case "/IPUNBAN":
                                ...
                                {
                                    bannedIPs.Remove(IPAddress.Parse(command.Substring(9)));
                                    Console.WriteLine($"[/IPUNBAN]: {clientName} ipunbanned {command.Substring(9)}");
                                }
                            ...
                            case "/OP":
                                ...
                                {
                                    adminList.Add(clients[command.Substring(4)]);
                                    Console.WriteLine($"[/OP]: {clientName} op'd {command.Substring(4)}");
                                }
                                catch { Console.WriteLine("[/OP]: Client not found"); }
                                break;
                            case "/UNOP":
                                try
                                {
                                    ...
                                    Console.WriteLine($"[/UNOP]: {clientName} unop'd {command.Substring(6)}");
                                }
                                catch { Console.WriteLine("[/UNOP]: Client not found"); }
                                break;
                            default:
                    ...
                    // NON ADMIN SERVER SIDE COMMANDS (has to be down here so the admin default statement from above isn't run after the client authenticates.)
                    switch (message.ToUpper().Split(' ')[0])
                    {
                        case "/DM":
                            try
        ...
        finally
        {
            ...
            {
                if (clientName != null && !isNameTaken)
                {
                    clients.TryRemove(clientName, out _);
    ...
    static void Broadcast(string message, string senderName)
    {
        ...
        {
            if (name.Key != senderName) // the sender does not need to see his messsage twice
            {
                TcpClient targetClient = name.Value;
    ...
    static void Main()
    {
        ...
        pwdHash = "0"; //admin login is disabled, because this is not a valid Sha512 hash.
        bannedNames.Add("*"); // * is used to apply commands to all users.
